...
## Dasar Teori
Dalam struktur data, linked list (senarai berkait) didefinisikan sebagai salah satu bentuk struktur data yang berisi kumpulan data yang tersusun secara sekuensial, saling bersambungan, dan bersifat dinamis [1]. Secara teori, linked list adalah sejumlah simpul (node) yang dihubungkan secara linier dengan bantuan pointer [1]. Setiap node umumnya berbentuk struct atau class [1]. Linked list memberikan keuntungan penting dibandingkan array, terutama dalam efisiensi pelaksanaan operasi penyisipan dan penghapusan data, serta penggunaan alokasi penyimpanan secara dinamis [1].

Bentuk dasar dari linked list adalah Single Linked List (SLL), di mana setiap simpul pembentuknya hanya mempunyai satu rantai (link) atau pointer ke simpul lainnya [1]. Sebuah node pada SLL umumnya memiliki dua bagian: medan informasi (berisi data yang akan disimpan dan diolah) dan medan penyambung (link field) yang berisi alamat atau pointer yang menunjuk ke simpul berikutnya [1]. Medan penyambung dari simpul terakhir akan bernilai null untuk menandakan akhir dari list [1].

Doubly Linked List (DLL) pada dasarnya memiliki konsep penggunaan yang hampir sama dengan SLL [1]. Perbedaan utamanya adalah bahwa Doubly Linked List menerapkan sebuah pointer baru, yaitu prev (previous) [1]. Pointer ini digunakan secara khusus untuk menunjuk ke simpul sebelumnya, yang memungkinkan proses pergeseran atau traversal data secara mundur [1]. Fitur ini melengkapi pointer next yang tetap dipertahankan untuk pergeseran atau traversal data secara maju [1].

Setiap simpul (node) pada DLL memiliki field yang berisi data serta dua pointer yang saling berhubungan, yaitu PREV (penunjuk ke data sebelumnya) dan NEXT (penunjuk ke data selanjutnya) [1]. Sebagai contoh, deklarasi struktur sebuah node DLL (disebut TNode dalam referensi) dapat terdiri dari tiga bagian: data (misalnya int data), sebuah pointer TNode *next, dan sebuah pointer TNode *prev [1]. Keberadaan dua pointer penunjuk (next dan prev) ini menjadikan Doubly Linked List lebih fleksibel dalam operasinya dibandingkan dengan Single Linked List [1]. Namun, fleksibilitas ini memiliki konsekuensi berupa kebutuhan alokasi memori tambahan untuk menyimpan pointer prev tersebut di setiap simpulnya [1].

Terdapat dua jenis DLL, yaitu Double Linked List Non Circular (DLLNC) dan Double Linked List Circular [1]. Pada tipe DLLNC, node saling terhubung melalui dua arah pointer [1]. Ciri khas dari DLLNC adalah pointer prev pada simpul pertama (head) dan pointer next pada simpul terakhir (tail) akan menunjuk ke NULL [1]. Untuk mengelola list, seringkali digunakan sebuah pointer head yang harus selalu menunjuk pada node pertama [1]. Saat inisialisasi awal untuk membuat list kosong, pointer head ini diatur ke NULL [1].
...
## Kesimpulan
Doubly Linked List adalah pengembangan dari Single Linked List yang menambahkan fungsionalitas signifikan melalui implementasi pointer kedua, yaitu prev. Penambahan pointer ini memungkinkan traversal data secara dua arah (maju menggunakan next dan mundur menggunakan prev), yang memberikan fleksibilitas lebih besar dalam operasi data dibandingkan SLL. Meskipun demikian, peningkatan fungsionalitas ini diimbangi dengan kebutuhan memori yang lebih besar, karena setiap simpul harus mengalokasikan ruang tambahan untuk menyimpan pointer prev. Implementasi DLL (seperti DLLNC)  sangat berguna dalam skenario di mana data perlu diakses atau dimanipulasi dari kedua arah secara efisien.
